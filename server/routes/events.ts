...
export function createEventsService(_pool: pg.Pool | null, apiKey: string): EventsService {
  ...
  const handleEventsRoutes = async (
    ...
    if (req.method === 'GET' && path === '/api/sports') {
      const names: Record<string, string> = { soccer: 'Futebol', tennis: 'Ténis', volleyball: 'Voleibol', rugby: 'Rugby', mma: 'MMA', 'ice-hockey': 'Hóquei no Gelo', handball: 'Andebol', basketball: 'Basquetebol' };
